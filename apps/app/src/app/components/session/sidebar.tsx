      ...
      <div class="flex-1 overflow-y-auto px-4 py-4 space-y-6">
        <div>
          <div class="flex items-center justify-between px-2 mb-2">
            <div class="text-xs text-gray-10 font-semibold uppercase tracking-wider">{t("session.sidebar_workspaces")}</div>
          </div>
          <div class="space-y-4">
            <Show
              ...
              fallback={
                <div class="px-3 py-2 rounded-lg border border-dashed border-gray-6 text-xs text-gray-9">
                  {t("session.sidebar_no_workspaces")}
                </div>
              }
            >
                      ...
                      <div class="flex items-start gap-2 px-2 py-2">
                              ...
                              <div class="flex items-center gap-2">
                                ...
                                <Show when={group.workspace.workspaceType === "remote"}>
                                  <span class="text-[9px] uppercase tracking-wide px-1.5 py-0.5 rounded-full bg-gray-3 text-gray-11">
                                    {isSandboxWorkspace() ? t("session.sidebar_sandbox") : t("session.sidebar_remote")}
                                  </span>
                                </Show>
                              </div>
                              <Show when={pathLabel()}>
                            ...
                            <div class="flex items-center gap-2 text-[10px] shrink-0">
                              ...
                              <Show when={!isConnecting() && connectionStatus() !== "connecting"}>
                                <Show when={connectionStatus() === "error"}>
                                  <span class="text-red-11 font-medium">{t("session.sidebar_needs_attention")}</span>
                                </Show>
                                <Show when={connectionStatus() !== "error"}>
                                  <Show when={isActive()} fallback={<span class="text-gray-9">{t("session.sidebar_switch")}</span>}>
                                    <span class="text-green-11 font-medium">{t("session.sidebar_active")}</span>
                                  </Show>
                                </Show>
                              </Show>
                            </div>
                        ...
                        <div class="flex flex-col items-center gap-1">
                          <button
                            ...
                            title={collapsed() ? t("session.sidebar_expand") : t("session.sidebar_collapse")}
                          >
                            <ChevronDown
                              size={14}
                          ...
                          <button
                            ...
                            title={t("session.sidebar_drag_reorder")}
                            draggable
                        ...
                        <div class="space-y-1 pl-2 pb-2">
                          ...
                          <div class="flex flex-wrap gap-2 px-3 pb-1">
                            <Show when={group.workspace.workspaceType === "remote"}>
                              ...
                              >
                                <Settings size={12} />
                                {t("session.sidebar_edit_connection")}
                              </button>
                              <button
                                type="button"
                                class="inline-flex items-center gap-1.5 rounded-md border border-gray-6 px-2 py-1 text-[10px] text-gray-10 hover:text-gray-12 hover:border-gray-7 hover:bg-gray-2 transition-colors"
                              ...
                              >
                                ...
                                {t("session.sidebar_test_connection")}
                              </button>
                            </Show>
                            <Show when={group.workspace.sandboxContainerName?.trim() && props.onStopSandbox}>
                              ...
                              >
                                <Square size={12} />
                                {t("session.sidebar_stop_sandbox")}
                              </button>
                            </Show>
                            <button
                              type="button"
                          ...
                          >
                            ...
                            <Show when={hasMoreSessions()}>
                              ...
                              >
                                {showingAll()
                                  ? t("session.sidebar_show_fewer")
                                  : t("session.sidebar_show_more").replace("{count}", String(sessions().length - MAX_SESSIONS_PREVIEW))}
                              </button>
                            </Show>
                          </Show>
                        </div>
            ...
            <div>
              ...
              >
                <Plus size={14} />
<<<<<<< HEAD
                {t("session.sidebar_add_workspace")}
              </button>
              <Show when={addWorkspaceMenuOpen()}>
                <div class="mt-2 rounded-lg border border-gray-6 bg-gray-1 shadow-lg overflow-hidden">
                  ...
                  >
                    <Plus size={12} />
                    {t("session.sidebar_connect_remote")}
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    <Plus size={12} />
                    {t("session.sidebar_import_config")}
                  </button>
                </div>
              </Show>
=======
        ...
        <div class="space-y-4">
          <Show when={realTodos().length > 0}>
            <div class="rounded-2xl border border-gray-6 bg-gray-2/30" id="sidebar-progress">
              ...
              >
                <span>{t("session.sidebar_progress")}</span>
                <ChevronDown
                  size={16}
        ...
        {(menu) => (
          ...
          >
            ...
            >
              ...
              >
                {t("session.sidebar_new_task_menu")}
              </button>
              <button
              ...
              >
                {t("session.sidebar_delete_session")}
              </button>
            </div>
          </div>
        )}
